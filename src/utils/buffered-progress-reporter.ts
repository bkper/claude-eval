...
import {
  ...
  ProgressLevel,
} from "./base-progress-reporter.js";
...
export class BufferedProgressReporter extends BaseProgressReporter {
  ...
  startEvaluation(filename: string): void {
    ...
    if (this.totalEvaluations > 1) {
      this.output(
        TerminalFormatter.formatHeader(
      ...
      this.output(
        TerminalFormatter.formatHeader(`Evaluating: ${filename}`, "section")
      );
    }
  }
  ...
  stepFailed(step: string, error?: string): void {
    ...
    this.output(
      TerminalFormatter.formatStep(`${step} failed${errorText}`, "error")
    );
  }
